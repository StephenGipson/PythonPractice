...
print(person.birthday())""",
                "hint": "Use self.name and self.age to store instance variables. Don't forget to use 'self' as the first parameter in all methods.",
                "test_cases": [
                    {
                        "test": "person = Person('Test', 20)\nassert 'Test' in person.introduce() and '20' in person.introduce()\nbirthday_msg = person.birthday()\nassert '21' in birthday_msg",
...
Create a Dog instance and test all methods.
                ...
                "hint": "Use super().__init__() to call the parent constructor. Override methods by defining them with the same name in the child class.",
                "test_cases": [
                    {
...
Note: We're simulating file operations for safety in this environment.
                ...
                "example": """students = [{"name": "Alice", "grade": 95}]
file_content = ""
...
for line in lines:
    ...
    print(f"Name: {name}, Grade: {grade}")""",
                "hint": "Use string formatting to create file content, then use split() and string parsing to read it back.",
                "test_cases": [
                    {
                        "test": "assert 'Alice' in file_content and 'Bob' in file_content and 'Grade:' in file_content",
...
print(list(squares))
print(sorted_scores)""",
                "hint": "Lambda syntax: lambda parameter: expression. Use x % 2 == 0 for even numbers, x ** 2 for squares, and x[1] for second element of tuple.",
                "test_cases": [
                    {
                        "test": "evens_list = list(evens)\nsquares_list = list(squares)\nassert 2 in evens_list and 4 in evens_list\nassert 1 in squares_list and 25 in squares_list\nassert sorted_students[0][1] < sorted_students[1][1]",
...
result = example_function()""",
                "hint": "Decorators are functions that take functions as input and return modified functions. Use func.__name__ to get the function name.",
                "test_cases": [
                    {
                        "test": "assert isinstance(result, int) and result > 0\n# Timer decorator should work and function should execute normally",
...
def fibonacci_generator(n):
    ...
    while count < n:
        ...
        a, b = b, a + b
        count += 1""",
                "hint": "Use 'yield' instead of 'return' to create generators. Fibonacci: start with a=0, b=1, then a, b = b, a+b.",
                "test_cases": [
                    {
                        "test": "fib_list = list(fibonacci_generator(5))\nassert fib_list[0] == 0 and fib_list[1] == 1 and fib_list[4] == 3\nsquares_list = list(even_squares(10))\nassert 4 in squares_list and 16 in squares_list",
...
with Timer():
    ...
    print(f"Result: {result}")""",
                "hint": "__enter__ is called when entering the 'with' block, __exit__ is called when leaving. Store start time in self.start.",
                "test_cases": [
                    {
